...
## 6b. Things a STATIC capture can never contain (learned on framer-ai.com)

Three failure modes that look like fidelity bugs but are really capture-scope bugs.
Each one was invisible in the artifacts and only showed up by diffing against the live
page — check for them on any new site.

**Hover-MOUNTED DOM.** Framer mega-menus do not exist in the resting DOM. At rest
framer-ai.com's header holds three 8×8 chevrons; hovering a nav item mounts a 1440×900
panel with a dozen links and ~90 KB of markup. `grep 'data-framer-name="Dropdowns"'
dom/full.html` → 0 hits, so the rebuild's dropdowns are simply dead. This is different
from `hover-thorough.json`, which captures style DELTAS on elements that already exist.
Fix: `extract-site capture-nav <url> --out reference/_nav` hovers each trigger with a
MutationObserver running and inlines the open state. Capture it ONCE per site — the
header is shared, so no page needs re-extracting.

**Not-yet-played entrances look identical to closed panels.** Both rest at opacity 0.
Guessing from resting opacity alone over-drops badly (223 of 276 appear effects on
`/agents`), and withholding a PARENT's entrance blanks its whole subtree — that page's
left icon rail renders solid black behind an opacity-0 `Side nav`. Fix:
`extract-site capture-entrances <ref>` scrolls the live page and decides from evidence.
Three things are required for that measurement to be worth anything:
- key on `framerName|WxH` and use only keys unique on BOTH sides. `data-cs-id` is a DFS
  index and shifts across page loads (3568 vs 3570 elements on `/agents` misaligned
  everything downstream while a count guard passed at 0.06% apart).
- use EFFECTIVE opacity, the product up the ancestor chain — the logo context menu sits
  at its own `opacity: 1` under a hidden parent.
- require SUSTAINED visibility (≥2 samples). Nav hover-highlights flash once during the
  header's entrance and then rest at 0.

**Scrollbars are baked into the geometry.** Chromium draws a classic 15px scrollbar, so a
100%-width element resolves to `width: 1425px` at a 1440px viewport and the rebuild pins
it there — every full-bleed section 15px narrow, centred content ~7px off. Launch capture
browsers with `--hide-scrollbars`. Detect it in existing references by FREQUENCY, not by
maximum: `body` legitimately measures the full 1440, so the signal is the most common
near-viewport width (1425 appearing 24 times against a single 1440).
