...
export async function captureSectionScreenshots(
  ...
  // CRITICAL: restore the desktop viewport. This loop leaves the page at the
  // LAST viewport (mobile, 390px) and every later phase reuses this same page
  // — motion linking, content, layout trees, hover, and scroll-linked motion
  // would all silently capture the MOBILE layout. Scroll-motion is the worst
  // hit: it reports "Mobile Nav"/"Mobile Menu" pinning and misses every
  // desktop scroll effect entirely.
  ...
  console.log(`  🧩 ${sections.length} sections × ${viewports.length} viewports (viewport restored → ${desktop?.name ?? 'n/a'} ${desktop?.width ?? '?'}px)`);
}
