...
// Deploy config that travels WITH the build output. netlify.toml only applies
// when the host builds from the repo; `_redirects`/`_headers` live in public/
// and so end up in dist/, which also covers deploying the folder directly.
// The SPA fallback is not optional here — the router is history-based, so
// without it every deep link 404s on refresh.
fs.writeFileSync(
  path.join(out, "public", "_redirects"),
  "/*  /index.html  200\n",
  "utf8",
);
fs.writeFileSync(
  path.join(out, "public", "_headers"),
...
// GENERATE the README rather than expecting a hand-written one to survive:
// `--force` wipes the output directory, so anything authored in there is lost
// on the next compose (it silently deleted this file once already).
{
  let origin = "the original site";
  try { origin = new URL(readJsonSafe(path.join(built[0].reference, "meta.json"))?.url ?? "").origin; } catch {}
  const routeList = built.map((p) => `\`${p.route}\``).join(" · ");
  fs.writeFileSync(
    path.join(out, "README.md"),
    `# Rebuilt site — ${built.length} route(s)

A clean, editable rebuild of ${built.length} page(s) from ${origin}, generated by
