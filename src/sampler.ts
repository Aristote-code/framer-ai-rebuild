...
export interface ScrollSample {
  ...
  viewportH: number;
  screenshot: string | null; // relative path, null when the shot was skipped
...
export async function sampledScrollCapture(
  ...
  while (i < maxSamples) {
    ...
    // NEVER let one sample screenshot abort the extraction. Playwright waits
    // for fonts before capturing, and a page with a stalled font request hangs
    // there until the 30s default fires — which killed a full 12-minute run of
    // framer-ai.com/stories at phase 1c. These sampler shots are a diagnostic
    // aid, not a rebuild input, so a missing frame costs nothing.
    let shotOk = true;
    try {
      await page.screenshot({ path: shotPath, fullPage: false, timeout: 12000 });
    } catch (e) {
      shotOk = false;
      console.warn(`  ⚠️  sample ${i} screenshot skipped: ${(e as Error).message.split('\n')[0]}`);
    }
    ...
    const sample: ScrollSample = {
      ...
      screenshot: shotOk ? `samples/screenshots/${shotName}` : null,
